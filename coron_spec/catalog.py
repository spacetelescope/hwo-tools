...
def load_csv(filename, label, planetary_radius=None, semimajor_axis=None, skiprows=0, angstroms=False, mult=1.0, lamhr_idx=0, Ahr_idx=1, magV=10, stellar_radius=1, stargalaxy=False):
    planet = {}
    fn = filename
    if stargalaxy:
        fn = os.path.join(relpath2, fn)
        ...
        fn = os.path.join(relpath, fn)
    model = np.loadtxt(fn, skiprows=skiprows, delimiter=",")
    lamhr_ = model[:,lamhr_idx]
    Ahr_ = model[:,Ahr_idx] * mult
    if angstroms:
        lamhr_ /= 10000.
...
def load_catalog():
    ...
    catalog_planet['False O2 Planet (orbiting F2V)']  = load_csv('fstarcloudy_geo_albedo_dedup.txt', 'Synthetic spectrum generated by S. Domagal-Goldman (Domagal-Goldman et al. 2014)', planetary_radius = 10000, semimajor_axis=1)
