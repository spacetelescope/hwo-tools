...
#~~~~~~~~~!!!!!!!!!~~~~~~
#NOTE!!!!!
#this has the distance issue with the M dwarf fixed (planet-star distance that is) 
#using Robinson et al parameters as discussed in the paper
...
# Instrument params
Res = 170. # vis resolution - SLIDER
Res_UV = 20. # UV resolution - SLIDER
Res_NIR = 170. #NIR resolution - SLIDER
De_UV = 5e-4 #dark current
De_VIS = 5e-4
De_NIR = 5e-4
Re_UV = 0.1 #read noise
Re_VIS = 0.1
Re_NIR = 0.1
...
fn = '../coronagraph/planets/ArcheanEarth_geo_albedo.txt'
model_c = np.loadtxt(fn, skiprows=8)
lamhr_c = model[:,0]
Ahr_c = model[:,1]    
...
lam, dlam, A, q, Cratio, cp, csp, cz, cez, cD, cR, cth, DtSNR = \
    cg.count_rates(Ahr, lamhr, solhr, alpha,  Rp, Teff, Rs, r, d, Nez, IWA=0.01, OWA=40., Tsys=270.,\
...
lam_M, dlam_M, A_M, q_M, Cratio_M, cp_M, csp_M, cz_M, cez_M, cD_M, cR_M, cth_M, DtSNR_M = \
    cg.count_rates(Ahr, lamhr, solhrM, alpha,  Rp, Teff2, RsM, rM, dm, Nez, IWA=0.01, OWA=40., Tsys=270.,\
...
#~~~~~~~~~~~~~~~~~~~
  ...
  #  print DtSNR
    ...
    # Set labels
    ax.set_ylabel("Integration Time Required for SNR = "+wantTsnr +" hours" , fontsize=14)
#    fig.set_xlabel("Wavelength [$\mu$m]", fontsize=18)
#    fig.set_title(title)
    fig.text(0.95, 0.2, plot_text,\
         verticalalignment='top', horizontalalignment='right',\
         transform=ax.transAxes,\
             color='black', fontsize=14)
    fig.text(0.95, 0.17, sun_text,\
         verticalalignment='top', horizontalalignment='right',\
         transform=ax.transAxes,\
             color='darkturquoise', fontsize=14)    
    fig.text(0.95, 0.14, gj_text,\
         verticalalignment='top', horizontalalignment='right',\
         transform=ax.transAxes,\
             color='mediumvioletred', fontsize=14)
    ...
    if ref_lam:
        ...
        plot_text = plot_text 
    # Draw plot
    sub1.plot(lam, DtSNR, lw=2.0, color="darkturquoise", alpha=1, ls="steps-mid")
    sub1.plot(lam_M, DtSNR_M, lw=2.0, color="mediumvioletred", alpha=1, ls="steps-mid")
    sub1.axis([0.3, 2.1, 1, 100000])
    sub1.semilogy()
    sub1.tick_params(labelsize=13)
    sub1.set_title('15.1 m segmented (12.7 m effective)', fontsize=17)
    sub1.text(.05, 0.00000001, "Integration Time Required for SNR = "+wantTsnr +" [hours]", rotation='vertical',  horizontalalignment='center', verticalalignment='center')
    sub1.axvline(1.64, color='red', linestyle='--')
    sub1.axvline(0.82, color='green', linestyle='--')
    sub1.axvline(0.54, color='blue', linestyle='--')

    
#    sub1.set_xlabel("Wavelength [$\mu$m]", fontsize=18)
    ...
    if ref_lam:
        ...
        plot_text = plot_text 
    # Draw plot
    sub2.plot(lam_7, DtSNR_7, lw=2.0, color="darkturquoise", alpha=1, ls="steps-mid")
    sub2.plot(lam_M_7, DtSNR_M_7, lw=2.0, color="mediumvioletred", alpha=1, ls="steps-mid")
    sub2.axis([0.3, 2.1, 1, 100000])
    sub2.semilogy()
    sub2.tick_params(labelsize=13)
    sub2.set_title('9 m segmented (7.6 m effective)', fontsize=17)
    sub2.axvline(0.98, color='red', linestyle='--')
    sub2.axvline(0.49, color='green', linestyle='--')
    sub2.axvline(0.33, color='blue', linestyle='--')
    ...
    if ref_lam:
        ...
        plot_text = plot_text 
    # Draw plot
    sub3.plot(lam_5, DtSNR_5, lw=2.0, color="darkturquoise", alpha=1, ls="steps-mid")
    sub3.plot(lam_M_5, DtSNR_M_5, lw=2.0, color="mediumvioletred", alpha=1, ls="steps-mid")
    sub3.axis([0.3, 2.1, 1, 100000])
    sub3.semilogy()
    sub3.tick_params(labelsize=13)
    sub3.set_title('6.5 m segmented (5.5 m effective)', fontsize=17)
    #fig.errorbar(lam, spec*1e9, yerr=sig*1e9, fmt='o', color='k', ms=5.0)
    sub3.axvline(0.71, color='red', linestyle='--')
    sub3.axvline(0.36, color='green', linestyle='--')
    sub3.axvline(0.24, color='blue', linestyle='--')
    sub3.axvline(1.97, color='blue')
    ...
    if ref_lam:
        ...
        plot_text = plot_text 
    # Draw plot
    sub4.plot(lam_4, DtSNR_4, lw=2.0, color="darkturquoise", alpha=1, ls="steps-mid")
    sub4.plot(lam_M_4, DtSNR_M_4, lw=2.0, color="mediumvioletred", alpha=1, ls="steps-mid")
    sub4.axis([0.3, 2.1, 1, 100000])
    sub4.semilogy()   
    sub4.set_title('4 m monolith', fontsize=17)
    sub4.tick_params(labelsize=13)
    sub4.set_xlabel("Wavelength [$\mu$m]", fontsize=14)
   # sub4.yaxis.set_ticks(np.arange(70000,80000,2500))
    #fig.errorbar(lam, spec*1e9, yerr=sig*1e9, fmt='o', color='k', ms=5.0)
    sub4.axvline(0.52, color='red', linestyle='--')
    sub4.axvline(0.26, color='green', linestyle='--')
    sub4.axvline(0.17, color='blue', linestyle='--')                
    sub4.axvline(1.43, color='blue')
    sub4.axvline(2.1, color='green')
