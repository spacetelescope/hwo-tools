...
def add_spectrum_to_library():
    ...
    filename = os.path.join(os.environ['PYSYN_CDBS'], 'etc', 'source', 'el_cww_fuv_001.fits')
    ...
    filename = os.path.join(os.environ['PYSYN_CDBS'], 'etc', 'source', 'sbc_cb2004a_001.fits')
    ell  = S.FileSpectrum(filename)
    e = ell.renorm(30., 'abmag', S.ObsBandpass('johnson,v'))
    e.convert('abmag') 
    e.convert('nm') 
    ...
    filename = os.path.join(os.environ['PYSYN_CDBS'], 'etc', 'source', 'sb1_kinney_fuv_001.fits')
    ell  = S.FileSpectrum(filename)
    e = ell.renorm(30., 'abmag', S.ObsBandpass('johnson,v'))
    e.convert('abmag') 
    e.convert('nm') 
