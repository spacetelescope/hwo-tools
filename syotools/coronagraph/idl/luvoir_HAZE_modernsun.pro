PRO LUVOIR_HAZE_MODERNSUN
  ...
  ;   spectrum
  tag = 'HAZE_modernsun'
  fn    = 'spectra/HAZE_modernsun.pt_toa.rad'
  ...
  IF(i[0] NE -1) THEN cont[i] = !VALUES.F_NAN ;<- spectrum cannot fall below zero
  IF(j[0] NE -1) THEN sigma[i] = !VALUES.F_NAN ;<- Infinite Errors!
  ...
  WRITECOL, 'output_files/luvoir_'+tag+'.txt', lam, Cratio, cont, sigma
